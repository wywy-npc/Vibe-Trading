...
- **2026-05-18** 🛡️ **Hard-gated research loop, marketdata cache, HITL inbox**: Three feature clusters land together. **(1)** ai-quant-lab integration: four tools (`critique`, `research_loop`, `validate_run`, `leakage_scan`), four skills (`adversarial-critic`, `strategy-discovery`, `strategy-validation`, `thesis-to-rules`), and a post-engine gate hook that writes `gates.json` and records every trial in `agent/research_memory.db` so the Deflated Sharpe counter is honest across sessions. The **hard-gate invariant** is now structural: `research_loop` stamps `via="loop"` + `loop_run_id` in `config.json`; `validate_run` refuses non-loop runs by default (override with `allow_non_loop=true`). See `docs/2026-05-18_aql_integration.md`. **(2)** New `agent/marketdata/` module with a DuckDB cache + Alpaca REST loader; `AAPL.US` now defaults to `alpaca` (requires `ALPACA_KEY_ID`/`ALPACA_SECRET_KEY`; falls back to yfinance silently if unset). **(3)** Checkpoint / HITL approvals system under `agent/src/checkpoint/` with a master-detail Inbox UI (`frontend/src/pages/Inbox.tsx`); gated by `ENABLE_HITL=true`. Reference skill: `thesis-to-rules` walks a PM through thesis interpretation → rule codification → backtest → sizing → deploy with four approval gates.
